...
from atlas import __app_name__, ROOT_DIR
from atlas.api.abstract.controller import SoftwareManager, ApplicationContext
...
def load_managers(locale: str, context: ApplicationContext, config: dict, default_locale: str, logger: Logger) -> List[SoftwareManager]:
    ...
    for f in os.scandir(f'{ROOT_DIR}/gems'):
        if f.is_dir() and f.name != '__pycache__':
            ...
            spec = importlib.util.find_spec(f'atlas.gems.{f.name}.controller')

            if spec:
                module = importlib.import_module(f'atlas.gems.{f.name}.controller')
